...
{
    tmp_dir=$1
    if [ -f ${FILE_LAST_KE_INFO} -a -f ${FILE_LAST_KMSG} ]; then
        cp -a ${FILE_LAST_KE_INFO} ${tmp_dir}/$(basename ${FILE_LAST_KE_INFO})
        cp -a ${FILE_LAST_KMSG} ${tmp_dir}/$(basename ${FILE_LAST_KMSG})
        ke_log=$(search_latest_file_with_pattern ${MINIDUMP_DIR} ${KE_FILE_PATTERN})
        if [ "x${ke_log}" != "x" ]; then
            cp -a  ${MINIDUMP_DIR}/${ke_log} ${tmp_dir}/${ke_log}
        fi
        rm -f ${FILE_LAST_KE_INFO}
        rm -f ${FILE_LAST_KMSG}
    fi
}
